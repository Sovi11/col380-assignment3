...
int** kruskal(int** maze, int n, int m){
    ...
    for (int i = 0; i < n; i++){
        for (int j = 0; j < m; j++){
            if (maze[i][j] == 1) vertices.push_back(make_pair(i,j));
        }
    }
    ...
    for (int i= 0 ;i <  vertices.size() ; i++){
        ...
        cout<< a << " " << b << endl;
        // if any of the neighbours has the same parent then dont add the vertex
        // check just of the neighbours not with self 
        if (a > 0 && new_matrix[a-1][b] == 1 && a < n-1 && new_matrix[a+1][b] == 1){
            if (find(parent, (a+1)*m + b) == find(parent, (a-1)*m + b)){
                continue;
            }
        }
        if (b > 0 && new_matrix[a][b-1] == 1 && b < m-1 && new_matrix[a][b+1] == 1){
            if (find(parent, a*m + b+1) == find(parent, a*m + b-1)){
                continue;
            }
        }
        if (a > 0 && new_matrix[a-1][b] == 1 && b > 0 && new_matrix[a][b-1] == 1){
            if (find(parent, a*m + b-1) == find(parent, (a-1)*m + b)){
                continue;
            }
        }
        if (a < n-1 && new_matrix[a+1][b] == 1 && b < m-1 && new_matrix[a][b+1] == 1){
            if (find(parent, a*m + b+1) == find(parent, (a+1)*m + b)){
                continue;
            }
        }
        if (a > 0 && new_matrix[a-1][b] == 1 && b < m-1 && new_matrix[a][b+1] == 1){
            if (find(parent, a*m + b+1) == find(parent, (a-1)*m + b)){
                continue;
            }
        }
        if (a < n-1 && new_matrix[a+1][b] == 1 && b > 0 && new_matrix[a][b-1] == 1){
            if (find(parent, a*m + b-1) == find(parent, (a+1)*m + b)){
                continue;
            }
        }
        // if no neighbour has the same parent then add the vertex
        if (a > 0){
            Union(parent, a*m + b, (a-1)*m + b);
        }
        if (a < n-1){
            Union(parent, a*m + b, (a+1)*m + b);
        }
        if (b > 0){
            Union(parent, a*m + b, a*m + b-1);
        }
        if (b < m-1){
            Union(parent, a*m + b, a*m + b+1);
        }
...
int main(){
    ...
    if (my_rank == 0) {
        ...
        int** maze_new = kruskal(maze, n, m);
        print_array(maze_new,n,m);
        // Use the maze...
